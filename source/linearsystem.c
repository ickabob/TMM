...
#ifdef DEBUG
  debug("Returning from step_linear_system after %d iterations\n",iteration);
#endif
  return;
}
...
#endif
  //do work
  for (r = t_no; r < dimension; r = r + NTHREADS) 
    {
      sum = 0.0;
      //inner product loop,
...
#ifdef DEBUG
  debug("T[%d] leaving thread_transformation.",t_no);
#endif
  //done doing work.
  pthread_exit(NULL);
}
